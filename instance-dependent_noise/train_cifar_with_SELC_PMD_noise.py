...
parser.add_argument('--noise_mode', default='PMD', help='feature dependent noise from paper PLC')
parser.add_argument('--noise_type', default='1', help='noise type from paper PLC, select from {1,2,3}')
parser.add_argument('--model', default='preActResnet34', type=str)
...
parser.add_argument('--r', default=0.35, type=float, help='noise ratio, select from {0.35, 0.70}')
...
# you need to download the PMD noisy labels in github
noisy_labels=np.load('/PMD_noisy_labels/'+args.dataset+'-'+args.noise_type+'-'+'%.2f' % args.r+'.npy')


if args.model == 'preActResnet34':
    model = PreResNet34(num_classes=args.num_class).to(args.gpuid)


if args.op == 'SGD':
    optimizer = optim.SGD(model.parameters(), lr=args.lr, momentum=0.9, weight_decay=5e-4)

if args.lr_s == 'MultiStepLR' and args.dataset == 'cifar10':
    scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[60, 120], gamma=0.2)
elif args.lr_s == 'MultiStepLR' and args.dataset == 'cifar100':
